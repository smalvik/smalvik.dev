...
**Docker networks:**
- `app` — nginx ↔ web, nginx ↔ api (Nginx проксирует запросы на оба сервиса)
- `db` — api ↔ postgres (изолированная сеть, БД доступна только из api)
- PostgreSQL **не публикует порты** на хост (`expose: 5432`, без `ports`)
- Подключение из api: `postgresql://user:pass@postgres:5432/smalvik` (Docker DNS по имени сервиса)
- Nginx — единственный контейнер с публичными портами (:80, :443)
- web и api не публикуют порты на хост, доступны только через nginx
